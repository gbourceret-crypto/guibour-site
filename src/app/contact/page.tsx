                ...
                <form ref={formRef} onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'0' }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1px', background:'#C8D8E8', marginBottom:'1px' }}>
                    <div style={{ background:'#fff', padding:'4px 0' }}>
                      <label style={{ display:'block', fontFamily:"'Orbitron', sans-serif", fontSize:'7px', color:'#8FA5B8', letterSpacing:'3px', padding:'10px 16px 0' }}>NOM / PSEUDO</label>
                      <input name="name" type="text" required placeholder="Votre pseudo..." style={{ width:'100%', border:'none', borderBottom:'2px solid transparent', fontFamily:"'Orbitron', sans-serif", fontSize:'13px', color:'#1A2B40', background:'transparent', padding:'6px 16px 10px', outline:'none', boxSizing:'border-box', transition:'border-color .2s' }}
                        onFocus={e => { e.target.style.borderBottomColor='#00C8BE'; }}
                        onBlur={e => { e.target.style.borderBottomColor='transparent'; }} />
                    </div>
                    <div style={{ background:'#fff', padding:'4px 0' }}>
                      <label style={{ display:'block', fontFamily:"'Orbitron', sans-serif", fontSize:'7px', color:'#8FA5B8', letterSpacing:'3px', padding:'10px 16px 0' }}>EMAIL</label>
                      <input name="email" type="email" required placeholder="votre@email.com" style={{ width:'100%', border:'none', borderBottom:'2px solid transparent', fontFamily:"'Orbitron', sans-serif", fontSize:'13px', color:'#1A2B40', background:'transparent', padding:'6px 16px 10px', outline:'none', boxSizing:'border-box', transition:'border-color .2s' }}
                        onFocus={e => { e.target.style.borderBottomColor='#00C8BE'; }}
                        onBlur={e => { e.target.style.borderBottomColor='transparent'; }} />
                    </div>
                  </div>
                  <div style={{ background:'#fff', marginBottom:'1px', padding:'4px 0' }}>
                    ...
                    <input name="subject" type="text" required placeholder="Sujet du message..." style={{ width:'100%', border:'none', borderBottom:'2px solid transparent', fontFamily:"'Orbitron', sans-serif", fontSize:'13px', color:'#1A2B40', background:'transparent', padding:'6px 16px 10px', outline:'none', boxSizing:'border-box', transition:'border-color .2s' }}
                      onFocus={e => { e.target.style.borderBottomColor='#00C8BE'; }}
                      onBlur={e => { e.target.style.borderBottomColor='transparent'; }} />
                  </div>
                  <div style={{ background:'#fff', marginBottom:'16px', padding:'4px 0' }}>
                    ...
                    <textarea name="message" required rows={6} placeholder="Votre message..." style={{ width:'100%', border:'none', borderBottom:'2px solid transparent', fontFamily:"'Orbitron', sans-serif", fontSize:'13px', color:'#1A2B40', background:'transparent', padding:'6px 16px 14px', outline:'none', resize:'none', boxSizing:'border-box', transition:'border-color .2s' }}
                      onFocus={e => { e.target.style.borderBottomColor='#00C8BE'; }}
                      onBlur={e => { e.target.style.borderBottomColor='transparent'; }} />
                  </div>
                  {error && (
                    <div style={{ fontFamily:"'Orbitron', sans-serif", fontSize:'10px', color:'#FF4444', letterSpacing:'1px', marginBottom:'10px', padding:'8px 12px', background:'rgba(255,68,68,.08)', border:'1px solid rgba(255,68,68,.2)' }}>
                      ⚠ {error}
                    </div>
                  )}
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div style={{ fontFamily:"'Orbitron', sans-serif", fontSize:'7px', color:'#C8D8E8', letterSpacing:'2px' }}>© 2026 GUIBOUR — W.O.W</div>
                    <button type="submit" disabled={sending} style={{ fontFamily:"'Lilita One', cursive", fontSize:'16px', letterSpacing:'4px', color:'#fff', background: sending ? '#3C5A7A' : 'linear-gradient(135deg,#0047AB,#007B8A)', border:'none', padding:'14px 40px', cursor: sending ? 'not-allowed' : 'pointer', boxShadow:'0 4px 18px rgba(0,71,171,.25)', transition:'all .2s ease', opacity: sending ? 0.7 : 1 }}
                      onMouseEnter={e => { if(!sending) { e.currentTarget.style.background='linear-gradient(135deg,#1B5EBB,#008B9A)'; e.currentTarget.style.boxShadow='0 6px 28px rgba(0,71,171,.4)'; } }}
                      onMouseLeave={e => { if(!sending) { e.currentTarget.style.background='linear-gradient(135deg,#0047AB,#007B8A)'; e.currentTarget.style.boxShadow='0 4px 18px rgba(0,71,171,.25)'; } }}
